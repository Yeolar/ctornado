...
public:
    ...
    static void _log(char level, const char *file, int line, int panic,
            const char *fmt, ...);
...
#define log_debug(...) do {                                             \
    if (Logger::loggable(Logger::DEBUG)) {                              \
        Logger::_log('D', __FILE__, __LINE__, 0, __VA_ARGS__);          \
    }                                                                   \
} while (0)

#define log_verb(...) do {                                              \
    if (Logger::loggable(Logger::VERB)) {                               \
        Logger::_log('V', __FILE__, __LINE__, 0, __VA_ARGS__);          \
    }                                                                   \
} while (0)
...
#define log_hexdump(_level, _data, _datalen, ...) do {                  \
    if (Logger::loggable(_level)) {                                     \
        Logger::_log('X', __FILE__, __LINE__, 0, __VA_ARGS__);          \
...
#define loga(...) do {                                                  \
    Logger::_log('-', __FILE__, __LINE__, 0, __VA_ARGS__);              \
} while (0)
...
#define log_critical(...) do {                                          \
    if (Logger::loggable(Logger::CRIT)) {                               \
        Logger::_log('C', __FILE__, __LINE__, 0, __VA_ARGS__);          \
    }                                                                   \
} while (0)
...
#define log_warn(...) do {                                              \
    if (Logger::loggable(Logger::WARN)) {                               \
        Logger::_log('W', __FILE__, __LINE__, 0, __VA_ARGS__);          \
    }                                                                   \
} while (0)

#define log_info(...) do {                                              \
    if (Logger::loggable(Logger::INFO)) {                               \
        Logger::_log('I', __FILE__, __LINE__, 0, __VA_ARGS__);          \
    }                                                                   \
} while (0)

#define log_panic(...) do {                                             \
    if (Logger::loggable(Logger::EMERG)) {                              \
        Logger::_log('P', __FILE__, __LINE__, 1, __VA_ARGS__);          \
    }                                                                   \
} while (0)
